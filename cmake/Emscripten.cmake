...
set(tcpp_WEB_DIR "${CMAKE_SOURCE_DIR}/web")
set(tcpp_COI_WORKER "${tcpp_WEB_DIR}/coi-serviceworker.min.js")
set(tcpp_SHELL_TEMPLATE "${tcpp_WEB_DIR}/shell_template.html.in")
set(tcpp_INDEX_TEMPLATE "${tcpp_WEB_DIR}/index_template.html.in")
...
if(EMSCRIPTEN)
  ...
  set(tcpp_WASM_INITIAL_MEMORY "33554432" CACHE STRING
      "Initial WASM memory in bytes (default: 32MB)")
  set(tcpp_WASM_PTHREAD_POOL_SIZE "4" CACHE STRING
      "Pthread pool size for WASM builds (default: 4)")
  set(tcpp_WASM_ASYNCIFY_STACK_SIZE "65536" CACHE STRING
...
function(tcpp_configure_wasm_target target)
  if(EMSCRIPTEN)
    ...
    set_property(GLOBAL APPEND PROPERTY tcpp_WASM_TARGETS "${target}")
    set_property(GLOBAL PROPERTY tcpp_WASM_TARGET_${target}_TITLE "${WASM_TITLE}")
    set_property(GLOBAL PROPERTY tcpp_WASM_TARGET_${target}_DESCRIPTION "${WASM_DESCRIPTION}")
    set_property(GLOBAL PROPERTY tcpp_WASM_TARGET_${target}_OUTPUT_NAME "${OUTPUT_NAME}")

    target_compile_definitions(${target} PRIVATE tcpp_WASM_BUILD=1)
    ...
    target_link_options(${target} PRIVATE
      ...
      "-sUSE_PTHREADS=1"
      "-sPROXY_TO_PTHREAD=1"
      "-sPTHREAD_POOL_SIZE=${tcpp_WASM_PTHREAD_POOL_SIZE}"
      # Enable asyncify for emscripten_sleep and async operations
      "-sASYNCIFY=1"
      "-sASYNCIFY_STACK_SIZE=${tcpp_WASM_ASYNCIFY_STACK_SIZE}"
      # Memory configuration
      "-sALLOW_MEMORY_GROWTH=1"
      "-sINITIAL_MEMORY=${tcpp_WASM_INITIAL_MEMORY}"
    ...
    if(EXISTS "${tcpp_SHELL_TEMPLATE}")
      ...
      message(FATAL_ERROR "Shell template not found: ${tcpp_SHELL_TEMPLATE}")
    endif()

    # Copy service worker to target build directory for standalone target builds
    if(EXISTS "${tcpp_COI_WORKER}")
...
function(tcpp_create_web_dist)
  ...
  get_property(WASM_TARGETS GLOBAL PROPERTY tcpp_WASM_TARGETS)
  ...
  foreach(target ${WASM_TARGETS})
    get_property(TITLE GLOBAL PROPERTY tcpp_WASM_TARGET_${target}_TITLE)
    get_property(DESCRIPTION GLOBAL PROPERTY tcpp_WASM_TARGET_${target}_DESCRIPTION)
...
")
  ...
  if(EXISTS "${tcpp_INDEX_TEMPLATE}")
    configure_file("${tcpp_INDEX_TEMPLATE}" "${INDEX_OUTPUT}" @ONLY)
  else()
    message(WARNING "Index template not found: ${tcpp_INDEX_TEMPLATE}")
  endif()
  ...
  foreach(target ${WASM_TARGETS})
    ...
    get_property(OUTPUT_NAME GLOBAL PROPERTY tcpp_WASM_TARGET_${target}_OUTPUT_NAME)
